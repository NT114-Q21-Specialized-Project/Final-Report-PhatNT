...
\noindent Sau bước khởi tạo mã nguồn hạ tầng, nhóm tiến hành kiểm tra lại đầu ra của lệnh apply, trạng thái các máy ảo đang hoạt động và cuối cùng là cấu trúc mạng đã được dựng lên trên AWS để bảo đảm trạng thái triển khai khớp với thiết kế mong muốn.

\begin{center}
    \includegraphics[width=0.88\textwidth]{graphics/chapter-5/5_1_infra/terraform_apply.png}
...
\end{itemize}
\noindent Trong khi đó, subnet \texttt{private-observability} được sử dụng cho Prometheus và Grafana. Việc định tuyến private subnet thông qua NAT Gateway giúp các máy nội bộ có thể truy cập ra ngoài khi cần cài đặt và cập nhật gói, nhưng vẫn giữ được nguyên tắc không phơi lộ trực tiếp ra Internet.
...
Một phần cấu hình chính của OpenVPN được nhóm khai báo như sau:
...
Ứng dụng được sử dụng trong đồ án là hệ thống \textit{Mini Ecommerce Microservices} do nhóm tự phát triển,  được xây dựng theo kiến trúc microservices đa ngôn ngữ nhằm phục vụ đồng thời cho thực nghiệm DevOps, observability và học tăng cường. Hệ thống mô phỏng một quy trình thương mại điện tử thu gọn, trong đó người dùng có thể đăng nhập, xem sản phẩm, kiểm tra tồn kho và tạo đơn hàng thông qua một điểm vào duy nhất là \texttt{api-gateway}.
...
Sau bước huấn luyện ngoại tuyến, mô hình được đóng gói và triển khai trở lại hệ thống dưới dạng một Pod chạy thường trực trên Kubernetes. Tác nhân được đặt trong namespace \texttt{rl-agent}, tách biệt khỏi workload nghiệp vụ nhưng vẫn có quyền can thiệp có kiểm soát lên namespace \texttt{mini-ecommerce}.

Các thành phần triển khai chính gồm:
\begin{itemize}
    \item \textbf{Namespace \texttt{rl-agent}}: cô lập tài nguyên của tác nhân RL.
    \item \textbf{ServiceAccount \texttt{rl-agent-sa}}: định danh thực thi cho Pod tác nhân.
    \item \textbf{Role và RoleBinding}: cấp quyền quan sát Pod, xóa Pod lỗi và cập nhật số lượng bản sao của deployment mục tiêu.
    \item \textbf{Deployment \texttt{rl-agent}}: duy trì một bản sao của tác nhân suy luận thời gian thực.
    \item \textbf{Kustomization}: gom manifest để triển khai đồng nhất bằng \texttt{kubectl apply -k}.
\end{itemize}

\noindent Ví dụ cấu hình gom manifest triển khai:
\begin{lstlisting}
...
resources:
  - namespace.yaml
  - serviceaccount.yaml
  - role-mini-ecommerce.yaml
  - rolebinding-mini-ecommerce.yaml
  - deployment.yaml
\end{lstlisting}

\noindent Ví dụ cấu hình quyền cho tác nhân RL:
\begin{lstlisting}
...
\noindent Trong deployment, tác nhân được truyền các biến môi trường và giới hạn tài nguyên quan trọng như sau:
\begin{lstlisting}
...
containers:
  - name: rl-agent
    env:
      - name: PROMETHEUS_URL
        value: "http://10.0.21.10:9090"
      - name: METRICS_NAMESPACE
        value: "mini-ecommerce"
      - name: POLL_INTERVAL_SEC
        value: "5"
      - name: ACTION_COOLDOWN_SEC
        value: "15"
      - name: MIN_REPLICAS
        value: "1"
      - name: MAX_REPLICAS
...
\noindent Cách triển khai này giúp mô hình RL hoạt động như một thành phần điều phối độc lập: liên tục thu thập số liệu từ Prometheus, suy luận hành động phục hồi và gửi lệnh can thiệp xuống Kubernetes API trong cùng môi trường vận hành thực tế.
