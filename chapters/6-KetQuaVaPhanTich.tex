Chương này trình bày kết quả huấn luyện và đánh giá các mô hình Reinforcement Learning được đề xuất trong bài toán tự phục hồi ứng dụng trên nền tảng Kubernetes. Ba thuật toán DQN, A2C và PPO được huấn luyện trên cùng môi trường mô phỏng và cùng tập dữ liệu trạng thái được xây dựng từ các kịch bản Pod Failure. Mục tiêu của quá trình thực nghiệm là đánh giá khả năng học chính sách phục hồi, mức độ hội tụ của từng thuật toán cũng như hiệu quả khi triển khai trên hệ thống thực tế.

Bên cạnh việc phân tích các chỉ số trong quá trình huấn luyện như phần thưởng tích lũy, giá trị hàm mất mát và xu hướng hội tụ, chương này còn đánh giá khả năng vận hành của các mô hình sau khi triển khai lên cụm Kubernetes. Các kết quả được so sánh dựa trên khả năng lựa chọn hành động phục hồi, tốc độ thích nghi với sự thay đổi của môi trường và hiệu quả khôi phục dịch vụ trong các kịch bản Pod Failure. Từ đó, đề tài đưa ra nhận xét về ưu điểm, hạn chế và mức độ phù hợp của từng thuật toán đối với bài toán tự phục hồi trên Kubernetes.
\section{Kết quả huấn luyện mô hình}

Trong phạm vi thực nghiệm hiện tại, dữ liệu huấn luyện được lấy từ các quỹ đạo artifact của kịch bản 
\texttt{chaos-pod-kill-product}. Các quỹ đạo này đã được ánh xạ về không gian trạng thái MDP và đưa 
vào môi trường \texttt{ArtifactReplayEnv} để huấn luyện tác nhân. Kết quả được lưu theo từng run, bao gồm 
mô hình, metadata và các biểu đồ chẩn đoán quá trình học.

\subsection{DQN}

Đối với DQN, mô hình được huấn luyện trong 400\,000 bước thời gian với môi trường động học 
\texttt{hybrid\_controlled}, hệ số nhiễu \texttt{noise\_scale = 0.70} và 40 episode đánh giá sau huấn luyện. 
Các siêu tham số chính của run này gồm \textit{learning rate} $= 5\times10^{-5}$, \textit{buffer size} = 200\,000, 
\textit{batch size} = 256, \textit{target update interval} = 2\,000 và cơ chế thăm dò \textit{epsilon-greedy} giảm từ 1.0 xuống 0.05.

\begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{graphics/chapter-6/train_DQN/training_curve.png}
...
\begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{graphics/chapter-6/train_DQN/loss_curve.png}
    \captionsetup{hypcap=false}
...
\begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=0.95\textwidth]{graphics/chapter-6/train_DQN/epsilon_curve.png}
    \captionsetup{hypcap=false}
...
\begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=0.95\textwidth]{graphics/chapter-6/train_DQN/dqn_diagnostics.png}
    \captionsetup{hypcap=false}
...
\begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{graphics/chapter-6/train_DQN/evaluation_summary.png}
    \captionsetup{hypcap=false}
...
\begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{graphics/chapter-6/train_A2C/policy_loss_curve.png}
    \captionsetup{hypcap=false}
...
\begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{graphics/chapter-6/train_A2C/value_loss_curve.png}
    \captionsetup{hypcap=false}
...
\begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{graphics/chapter-6/train_PPO/learning_rate_curve.png}
    \captionsetup{hypcap=false}
...
\begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{graphics/chapter-6/train_PPO/loss_curve.png}
    \captionsetup{hypcap=false}
...
\begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{graphics/chapter-6/train_PPO/evaluation_summary.png}
    \captionsetup{hypcap=false}
...
\begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{graphics/chapter-6/compare/action_distribution.png}
    \captionsetup{hypcap=false}
...
\begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{graphics/chapter-6/compare/system_metrics_summary.png}
    \captionsetup{hypcap=false}
